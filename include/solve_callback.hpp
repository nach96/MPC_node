...
# endif
	...
	*/
	...
	{	size_t j;
		...
		CPPAD_ASSERT_UNKNOWN(static_cast<size_t>(n) == nx_ );
		CPPAD_ASSERT_UNKNOWN(static_cast<size_t>(m) == ng_ );
		if(solution_.x.size()>0 and init_z ==true){
            		CPPAD_ASSERT_UNKNOWN(init_x == true);
            		CPPAD_ASSERT_UNKNOWN(init_z == true);
            		CPPAD_ASSERT_UNKNOWN(init_lambda == true);
            		//Should shift all of them one step. Try first like this.
			for(j = 0; j < nx_; j++){
				x[j] = solution_.x[j];
			}
			for(j = 0; j < solution_.zl.size(); j++){
				z_L[j] = solution_.zl[j];
			}
			for(j = 0; j < solution_.zu.size(); j++){
				z_U[j] = solution_.zu[j];
			}
			for(j = 0; j < solution_.lambda.size(); j++){
				lambda[j] = solution_.lambda[j];
			}
